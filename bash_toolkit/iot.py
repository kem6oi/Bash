...
def mqtt_client(topic, mode="subscribe", broker="localhost", port=1883, message=None, qos=0, retain=False, username=None, password=None, client_id=None, output_file=None, dry_run=False):
    """
    Wrapper for bash_toolkit/scripts/iot/mqtt_helper.sh
    """
    ...
    return run_script("bash_toolkit/scripts/iot/mqtt_helper.sh", args, stream=stream, dry_run=dry_run)

def device_discovery(scan_time=5, services=None, all_services=False, continuous=False, output_file=None, dry_run=False):
    """
    Wrapper for bash_toolkit/scripts/iot/device_discovery.sh
    """
    args = ["-t", scan_time]
    ...
    return run_script("bash_toolkit/scripts/iot/device_discovery.sh", args, stream=continuous, dry_run=dry_run)

def firmware_check(device_type, current_version, update_url=None, output_file=None, dry_run=False):
    """
    Wrapper for bash_toolkit/scripts/iot/firmware_checker.sh
    """
    ...
    return run_script("bash_toolkit/scripts/iot/firmware_checker.sh", args, dry_run=dry_run)

def gpio_control(pin, action="read", value=None, mode="out", dry_run=False):
    """
    Wrapper for bash_toolkit/scripts/iot/gpio_controller.sh
    """
    ...
    return run_script("bash_toolkit/scripts/iot/gpio_controller.sh", args, dry_run=dry_run)

def serial_monitor(port="/dev/ttyUSB0", baudrate=9600, output_file=None, no_timestamps=False, dry_run=False):
    """
    Wrapper for bash_toolkit/scripts/iot/serial_monitor.sh
    """
    ...
    return run_script("bash_toolkit/scripts/iot/serial_monitor.sh", args, stream=True, dry_run=dry_run)

def traffic_monitor(interface="eth0", filter_exp=None, packet_count=100, duration=0, output_file=None, dry_run=False):
    """
    Wrapper for bash_toolkit/scripts/iot/traffic_monitor.sh
    """
    ...
    return run_script("bash_toolkit/scripts/iot/traffic_monitor.sh", args, stream=True, dry_run=dry_run)
